...
const Sidebar: React.FC<DashboardSidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 bottom-0 right-0 lg:left-0 lg:right-auto lg:top-16 lg:bottom-0 z-40 w-64 bg-white border-l-2 lg:border-l-0 lg:border-r-2 border-red-600 flex flex-col justify-between transition-transform duration-300 ease-in-out lg:translate-x-0 lg:overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${isOpen ? "translate-x-0 shadow-2xl shadow-red-950/20" : "translate-x-full lg:translate-x-0 lg:shadow-none"
          }`}
      >
        <div className="py-3">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={onClose}
                  className={`relative flex items-center justify-between px-3.5 py-2.5 text-xs sm:text-sm font-extrabold transition-all duration-150 border-2 ${isActive
                ...
                >
                  <div className="flex items-center gap-3">
                    <Icon
                      className={`w-5 h-5 stroke-[2.2] ${isActive ? "text-white" : "text-slate-600 group-hover:text-red-600"
                        }`}
                    />
